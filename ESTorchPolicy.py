...
from ESTorchAlgo import ESConfig
...
def before_init(policy, observation_space, action_space, config):
    ...
    def _compute_actions(policy, obs_batch, add_noise=False, update=False, **kwargs):
    ...
    def _compute_single_action(
        policy, observation, add_noise=False, update=False, **kwargs
    ):
...
def make_model_and_action_dist(policy, observation_space, action_space, config):
    ...
    model = ModelCatalog.get_model_v2(
        ...
        # int(np.prod(np.array(policy.preprocessor.observation_space.shape))),
        # Box(0, 255, 9216, uint8),
        action_space,
